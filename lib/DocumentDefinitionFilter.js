...
class TypeMap {
  ...
  addType(type) {
    ...
    //TODO: handle operation definitions?

    if (type.kind === graphql.Kind.OBJECT_TYPE_DEFINITION || type.kind === graphql.Kind.OBJECT_TYPE_EXTENSION) {
...
class DocumentDefinitionFilter {
  ...
  static addTransitiveTypes(definition) {
    ...
    else if (definition.kind === graphql.Kind.ENUM_TYPE_DEFINITION || definition.kind === graphql.Kind.ENUM_TYPE_EXTENSION) {
      //Visit the directives
      dependencies.push(...definition.directives.map(directive => directive.name.value));
  ...
  filter(document, otherDependencies, types) {
    ...
    const { schema, extensions } = typeMap.getSchema();

    if (schema) {
      for (const directive of schema.directives) {
        dependencies.add(directive.name.value);
      }
      for (const schemaExtensions of extensions) {
        for (const directive of schemaExtensions.directives) {
          dependencies.add(directive.name.value);
        }
      }
    }
    ...
    return graphql.visit(mergedDocument, {
      enter(node) {
        if (node.kind === graphql.Kind.SCALAR_TYPE_DEFINITION) {
          console.log();
        }
